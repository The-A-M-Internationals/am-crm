  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard label="Total Leads"     value={leads.length}    sub={`${wonLeads} won ┬╖ ${lostLeads} lost`} color="#C9A84C" icon="≡ƒôè" />
        <StatCard label="Active Clients"  value={clients.length}  sub="Ongoing relationships"                color="#3b82f6" icon="≡ƒæÑ" />
        <StatCard label="Open Projects"   value={projects.filter(p => p.status !== "completed").length} sub="In progress" color="#8b5cf6" icon="≡ƒÜÇ" />
        <StatCard label="Pending Tasks"   value={tasks.length}    sub="Across all team"                      color="#f59e0b" icon="Γ£à" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="crm-card">
          <div className="flex items-center justify-between mb-4">
            ...
            <a href="/leads" className="text-xs font-semibold" style={{ color: "#C9A84C" }}>View all ΓåÆ</a>
          </div>
          {loading ? (
        ...
        <div className="crm-card">
          <div className="flex items-center justify-between mb-4">
            ...
            <a href="/tasks" className="text-xs font-semibold" style={{ color: "#C9A84C" }}>View all ΓåÆ</a>
          </div>
          {loading ? (
            <div className="space-y-2">{[1,2,3].map(i => <div key={i} className="h-10 rounded-lg animate-pulse" style={{ background: "#f0f2f8" }} />)}</div>
          ) : tasks.length === 0 ? (
            <div className="text-center py-6">
              <p className="text-xs" style={{ color: "#9ca3af" }}>All tasks done! ≡ƒÄë</p>
            </div>
          ) : (
            <div className="space-y-2">
                  ...
                  <div key={task.id} className="flex items-start gap-3 p-2.5 rounded-xl hover:bg-gray-50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        {task.clientName && <p className="text-xs truncate" style={{ color: "#9ca3af" }}>≡ƒÅó {task.clientName}</p>}
                        {task.dueDate && <p className="text-xs" style={{ color: isOverdue ? "#ef4444" : "#9ca3af" }}>≡ƒôà {new Date(task.dueDate).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</p>}
                      </div>
                    </div>
