...
export async function POST(req: Request) {
  try {
    ...
    if (!process.env.RESEND_API_KEY) {
      console.warn("[Proposal] RESEND_API_KEY is not configured. Simulating email send for dev environment.");
    ...
    if (!proposal) {
      try {
        ...
        const docRef = adminDb.collection("proposals").doc(proposalId);
        ...
        console.warn("Could not fetch proposal using adminDb, falling back or failing:", err.message);
