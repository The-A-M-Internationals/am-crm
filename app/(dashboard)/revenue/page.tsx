...
export default function RevenuePage() {
  ...
  //fetching exchangerates for currency
  ...
  const manualRevenueTotal = manualRev.reduce((s, r) => s + convertToAED(Number(r.amount) || 0, r.currency || "AED"), 0);
  ...
  const clientInvoices = selectedClient
    ...
    : [];
  const clientBudget = clientInvoices.reduce(
    (sum, inv) => sum + (Number(inv.total) || 0),
    0,
  );
  const clientPaid = clientInvoices
    .filter((inv) => inv.status === "paid")
    .reduce((sum, inv) => sum + (Number(inv.total) || 0), 0);
  const clientPending = clientInvoices
    .filter((inv) => inv.status !== "paid")
    .reduce((sum, inv) => sum + (Number(inv.total) || 0), 0);
        ...
        <>
          ...
          {selectedClient && (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              ...
              <div className="stat-card">
                ...
                <p className="text-lg font-bold">{clientInvoices.length}</p>
              </div>
            </div>
          )}
          {/* Monthly chart */}
          <div className="crm-card mb-6">
            ...
            <p className="text-xs mb-5" style={{ color: "#9ca3af" }}>
              Gold = Revenue · Red = Expenses · Navy = Profit
            </p>
            ...
            <div className="crm-card">
              ...
              {clientData.length === 0 ? (
                <div className="text-center py-10">
                  ...
                  <p className="text-xs mt-1" style={{ color: "#c4c7d0" }}>
                    Mark invoices as "Paid" in the Invoices tab
                  </p>
                </div>
              ) : (
