    ...
    try {
      ...
      if (editingId) {
        ...
        if (form.status === "accepted") {
          await PipelineService.handleProposalStatusChange(
            { id: editingId, ...data },
            "accepted",
          );
        }
    ...
    <div className="p-8 max-w-[1400px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm font-medium text-slate-500 mt-1">
            Manage and track your proposal pipeline
          </p>
        </div>
        <button
          onClick={startAdding}
          className="btn-primary shadow-lg shadow-blue-900/20 py-2.5 px-6 rounded-xl hover:-translate-y-0.5 transition-all"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-10">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col relative overflow-hidden transition-all hover:shadow-md">
          <div className="absolute -right-4 -top-4 p-8 bg-blue-50 text-blue-500 rounded-full opacity-50">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
          ...
          <p className="text-sm font-bold text-slate-500 mb-2 z-10">
            Total Proposals
          </p>
          <p className="text-4xl font-black text-[#0D1B3E] z-10">
            {proposals.length}
          </p>
        </div>
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col relative overflow-hidden transition-all hover:shadow-md">
          <div className="absolute -right-4 -top-4 p-8 bg-emerald-50 text-emerald-500 rounded-full opacity-50">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
          ...
          <p className="text-4xl font-black text-emerald-600 z-10">
            {proposals.filter((p) => p.status === "accepted").length}
          </p>
        </div>
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col relative overflow-hidden transition-all hover:shadow-md">
          <div className="absolute -right-4 -top-4 p-8 bg-amber-50 text-amber-500 rounded-full opacity-50">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
          ...
          <p className="text-4xl font-black text-[#C9A84C] z-10 tracking-tight">
            <span className="text-xl font-bold text-amber-600/70 mr-1">
              AED
            </span>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col relative overflow-hidden transition-all hover:shadow-md">
          <div className="absolute -right-4 -top-4 p-8 bg-purple-50 text-purple-500 rounded-full opacity-50">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
      ...
      {(isAddingNew || editingId) && (
        <div
          ref={activeRef}
          className="bg-white p-6 rounded-2xl shadow-xl mb-8 border border-slate-200 font-sans relative"
        >
          <div className="absolute top-0 left-0 w-full h-1 bg-[#C9A84C] rounded-t-2xl"></div>
          <h2 className="text-xl font-bold mb-6" style={{ color: "#0D1B3E" }}>
            {editingId ? "Edit Proposal Details" : "Create New Proposal"}
          </h2>
          <ProposalForm
            form={form}
            setForm={setForm}
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-28 rounded-2xl animate-pulse bg-slate-100"
            />
          ))}
        </div>
        ...
        <div className="text-center py-24 bg-white rounded-2xl border border-slate-200 shadow-sm">
          ...
          <p className="text-sm text-slate-500 mb-6 max-w-sm mx-auto">
            Get started by creating your first sales proposal. It only takes a
            minute.
          </p>
          <button
            onClick={startAdding}
            className="btn-primary mx-auto py-2.5 px-6"
          >
            Create Proposal
          </button>
        </div>
        ...
        <div className="space-y-4">
          {proposals.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className="bg-white rounded-2xl transition-all hover:shadow-lg border border-slate-200 hover:border-[#C9A84C] relative group"
              >
                <div
                ...
                <div
                  className="p-5 pl-7 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 cursor-pointer"
                  onClick={() => viewProposal(p.id)}
                >
                  {/* Left: Client & Core Info */}
                  <div className="flex items-center gap-5 flex-1 min-w-0">
                    <div
                      className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 text-white font-black text-xl shadow-sm"
                      style={{
                        background: `linear-gradient(135deg, ${st.color} 0%, #0D1B3E 100%)`,
                      }}
                    >
                      {p.clientName?.charAt(0).toUpperCase() || "?"}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="text-lg font-bold truncate text-[#0D1B3E] group-hover:text-blue-700 transition-colors">
                        {p.clientName}
                      </h3>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <span
                          ...
                          style={{
                            background: svc?.bg || "#f3f4f6",
                            color: svc?.text || "#374151",
                          }}
                        >
                          {svc?.label || "Unknown Service"}
                        </span>
                  ...
                  <div className="flex flex-col items-start lg:items-end justify-center min-w-[140px]">
                    <div className="flex items-baseline gap-1">
                      ...
                      <span className="text-2xl font-black text-[#0D1B3E]">
                    ...
                    <span
                      className="text-xs font-bold px-3 py-1 rounded-full mt-1.5 border"
                      style={{
                        background: st.bg,
                        color: st.color,
                  ...
                  {/* Right: Actions */}
                  <div
                    className="flex items-center gap-2 lg:opacity-0 group-hover:opacity-100 transition-opacity w-full lg:w-auto"
                    ...
                    <button
                      ...
                      onClick={async (e) => {
                        ...
                        try {
                          ...
                          alert(
                            "Proposal sent successfully to " + p.clientEmail,
                          );
                        } catch (err) {
                      ...
                      {openMenu === p.id && (
                        ...
                        >
                          <div className="sm:hidden">
                            <button
                              className="w-full text-left px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50"
                              onClick={() => {
                                setOpenMenu(null);
                                viewProposal(p.id);
                              }}
    ...
    <div className="space-y-6">
          ...
          {SERVICES.map((s) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-[11px] font-bold ${isSelected ? "text-blue-900" : "text-slate-600"}`}
                >
                  {s.label}
                </span>
              </div>
      ...
      <div className="border-t border-slate-100 pt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="form-label text-xs">Client Email *</label>
            <input
              className="form-input py-2.5"
      ...
      <div className="grid grid-cols-3 gap-3 mt-3">
        <div>
          ...
          <select
            ...
            value={form.status}
            onChange={(e) =>
              setForm({ ...form, status: e.target.value as ProposalStatus })
            }
          >
            {STATUSES.map((s) => (
      ...
      <div>
        ...
        >
          <div
            className="grid text-[10px] font-bold uppercase tracking-wide px-3 py-2"
            style={{
              gridTemplateColumns: "1fr 60px 100px 100px 30px",
              gap: 8,
              background: "#f8f9fc",
              color: "#9ca3af",
            }}
          ...
          {form.items.map((item: any, i: number) => (
            <div
              key={i}
              className="grid items-center px-3 py-1.5 border-t"
              style={{
                gridTemplateColumns: "1fr 60px 100px 100px 30px",
                gap: 8,
                borderColor: "#f0f0f5",
        ...
        <div className="mt-3 flex flex-col items-end space-y-1 text-xs font-medium">
          ...
          <div
            className="flex gap-4 text-sm font-bold pt-1 border-t"
            style={{ color: "#C9A84C", borderColor: "#e5e7eb" }}
          >
            <span>Total:</span>
            <span>
              {form.currency}{" "}
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button
          onClick={cancelEdit}
          className="px-6 py-2 rounded-xl border text-sm font-semibold"
