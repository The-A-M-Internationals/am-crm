...
export default function InvoicePage() {
  ...
  function openAdd() {
    setEditing(null);
    setForm({ ...EMPTY_FORM, items: [{ description: "", qty: 1, rate: 0, amount: 0 }] });
    setShowModal(true);
  }
  ...
  async function downloadPDF(inv: any) {
    try {
      ...
      setPreview(inv); setShowPreview(true);
  ...
  async function sendEmail(inv: any) {
    if (!inv.clientEmail) {
      alert("❌ No client email on this invoice! Please edit the invoice and add a client email first.");
      return;
    }
    setSending(true);
    try {
      ...
      if (result.error) {
        ...
        alert("⚠️ Email skipped — RESEND_API_KEY not configured in environment variables.");
      } else {
                ...
                <div className="flex items-center justify-between flex-wrap gap-3">
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div>
                      ...
                      <p className="text-xs mt-0.5" style={{ color: "#9ca3af" }}>
                        {svc?.label} · {new Date(inv.createdAt).toLocaleDateString("en-GB")}
                        {inv.dueDate && ` · Due ${new Date(inv.dueDate).toLocaleDateString("en-GB")}`}
                      </p>
                      {inv.clientEmail && (
                        <p className="text-xs mt-0.5" style={{ color: "#9ca3af" }}>📧 {inv.clientEmail}</p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2 flex-wrap flex-shrink-0">
                    ...
                    {STATUSES.filter(s => s.key !== inv.status).map(s => (
                      <button key={s.key} onClick={() => updateStatus(inv, s.key)} className="badge cursor-pointer hover:opacity-80" style={{ background: s.bg, color: s.color, border: `1px solid ${s.border}` }}>
                        → {s.label}
                      </button>
                    ))}
          ...
          <div className="modal-box" style={{ maxWidth: 680 }}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div><label className="form-label">Client Name *</label><input className="form-input" value={form.clientName} onChange={e => setForm({ ...form, clientName: e.target.value })} placeholder="Client or company name" /></div>
                <div><label className="form-label">Client Email</label><input className="form-input" type="email" value={form.clientEmail} onChange={e => setForm({ ...form, clientEmail: e.target.value })} placeholder="client@email.com" /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
        ...
        <div className="modal-overlay" onClick={() => setShowPreview(false)}>
          <div style={{ width: "100%", maxWidth: 700, maxHeight: "90vh", overflowY: "auto", borderRadius: 16 }} onClick={e => e.stopPropagation()}>
            <div id="invoice-print-area" style={{ background: "white", padding: "40px", fontFamily: "Arial,sans-serif" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 32, paddingBottom: 24, borderBottom: "2px solid #f0f0f5" }}>
                <div>
                  ...
                  <p style={{ margin: 0, fontSize: 11, color: "#9ca3af" }}>am@theaminternationals.com · +91 90255 62311</p>
                  <p style={{ margin: 0, fontSize: 11, color: "#C9A84C" }}>theaminternationals.com</p>
                </div>
              ...
              {preview.notes && (
                <div style={{ padding: "14px", background: "#f8f9fc", borderRadius: 8, marginBottom: 24, borderLeft: "3px solid #C9A84C" }}>
                  <p style={{ margin: 0, fontSize: 11, color: "#6b7280", fontWeight: 700 }}>NOTES</p>
                  <p style={{ margin: "4px 0 0", fontSize: 12, color: "#374151" }}>{preview.notes}</p>
                </div>
              )}
              ...
              <div style={{ textAlign: "center", paddingTop: 20, borderTop: "1px solid #f0f0f5" }}>
                ...
                <p style={{ margin: "4px 0 0", fontSize: 11, color: "#C9A84C" }}>The A&M Internationals FZC · Ajman Free Zone, UAE · theaminternationals.com</p>
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 10, padding: 16, background: "white", borderTop: "1px solid #f0f0f5", borderRadius: "0 0 16px 16px" }}>
              ...
              <button onClick={() => sendEmail(preview)} disabled={sending} className="btn-primary" style={{ flex: 1, justifyContent: "center", background: "#1d4ed8" }}>✉ Send Email</button>
