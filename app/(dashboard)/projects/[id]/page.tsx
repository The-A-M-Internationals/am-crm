"use client";
import { X, Trash2, ClipboardList, Search, Mail, Building2, DollarSign, Pencil, User, Bell, Upload, FileText, FileSpreadsheet, Presentation, File, Download, Paperclip, CheckCircle2, Users } from "lucide-react";
...
export default function ProjectDetailsPage({ params }: { params: { id: string } }) {
  ...
  async function updateTaskInstructions(task: any, instructions: string) {
    try {
      ...
      setProjectTasks(prev => prev.map(t => t.id === task.id ? { ...t, taskInstructions: instructions, description: instructions } : t));
  ...
  return (
    <div className="p-8">
                          ...
                          <div onClick={e => e.stopPropagation()}>
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                else if (action === "note") { setNoteModalTask(task); setNoteModalText(task.taskInstructions || task.description || ""); setShowNoteModal(true); }
      ...
      {/* Custom Add Note / Instructions Modal */}
      {showNoteModal && noteModalTask && (() => {
        const assignedAssignees = users.filter(u => 
          Array.isArray(noteModalTask?.assignedTo) 
            ? noteModalTask.assignedTo.includes(u.uid)
            : noteModalTask?.assignedTo === u.uid
        );

        return (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-3xl p-6 sm:p-7 max-w-lg w-full shadow-2xl border border-slate-100 animate-in zoom-in-95 duration-200">
              {/* Header */}
              <div className="flex items-start justify-between gap-3 mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-2xl bg-[#0D1B3E] text-[#C9A84C] flex items-center justify-center shadow-sm shrink-0">
                    <ClipboardList className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="text-base font-bold text-slate-900 leading-snug">Task Directives & Instructions</h3>
                    <p className="text-xs text-slate-500">Provide clear guidelines and expectations for this task.</p>
                  </div>
                </div>
                <button 
                  onClick={() => {
              ...
              {/* Task Context Card */}
              <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 mb-4 space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs font-bold text-slate-900 truncate flex items-center gap-2">
                    ...
                    <span className="truncate">{noteModalTask.title}</span>
                  </div>
                  <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full border border-blue-200 bg-blue-50 text-blue-700 shrink-0">
                    {noteModalTask.priority || "medium"}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-1.5 text-[11px] text-slate-500">
                  ...
                  {assignedAssignees.length > 0 ? (
                    <span className="truncate text-slate-700 font-medium">
                      {assignedAssignees.map(a => a.name).join(", ")}
                    </span>
                  ) : (
                    <span className="text-slate-400 italic">No assignees yet</span>
                  )}
                </div>
              </div>
              ...
              {/* Input Area */}
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1.5">Directives & Deliverables</label>
                <textarea 
                  className="w-full bg-slate-50/50 hover:bg-white border border-slate-200 rounded-2xl p-3.5 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-[#C9A84C] focus:ring-2 focus:ring-[#C9A84C]/20 min-h-[140px] resize-y shadow-inner font-sans leading-relaxed transition-all"
                  placeholder="Type clear instructions, guidelines, reference links, or deliverables here..."
                  value={noteModalText}
                ...
                <div className="flex items-center justify-between mt-1.5 px-1 text-[10px] text-slate-400 font-medium">
                  <span>Supports multi-line directives and URLs</span>
                  <span>{noteModalText.length} characters</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
                <button 
                  ...
                  className="px-4 py-2.5 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-600 text-xs font-semibold transition-all"
                >
                  Cancel
                </button>
                <button 
                  onClick={() => {
                    if (noteModalTask) {
                      updateTaskInstructions(noteModalTask, noteModalText.trim());
                    }
                    setShowNoteModal(false);
                    setNoteModalText("");
                    setNoteModalTask(null);
                  }} 
                  className="px-5 py-2.5 bg-[#0D1B3E] hover:bg-[#1a3070] text-white text-xs font-bold rounded-xl shadow-md hover:shadow-lg transition-all flex items-center gap-1.5"
                >
