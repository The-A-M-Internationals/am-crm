...
export default function ProjectDetailsPage({
  ...
  const [selectedDrawerTask, setSelectedDrawerTask] = useState<any | null>(
    null,
  );
  const [logInputText, setLogInputText] = useState("");
  const [duplicateConflictTask, setDuplicateConflictTask] = useState<
    any | null
  >(null);
  ...
  const [activeCategoryFilter, setActiveCategoryFilter] = useState<
    "All" | "Design" | "Development" | "Documentation" | "Credentials"
  >("All");
  ...
  useEffect(() => {
    ...
    const unsubTasks = onSnapshot(qTasks, (snap) => {
      ...
      ghosts.forEach(async (ghostId) => {
        try {
          await deleteDoc(doc(db, "tasks", ghostId));
        } catch (e) {}
      });
    });
  ...
  async function delegateTask() {
    ...
    const titleCollision = projectTasks.find(
      (t) =>
        t.relatedTo === project.id &&
        t.title.trim().toLowerCase() ===
          delegateForm.title.trim().toLowerCase(),
    );

    if (titleCollision) {
      alert(
        "WARNING: A task with this exact title already exists on this project. Please append a sequence index (e.g., V2, Part 2) to ensure separate communication history sheets.",
      );
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        description:
          delegateForm.instructions || `Task for project: ${project.title}`,
  ...
  async function updateTaskStatus(task: any, status: string) {
    if (crmUser?.role === "admin") {
      alert(
        "Action Restricted: Only the assigned Employee or Lead can update active task milestones.",
      );
      return;
    }
    try {
  ...
  async function commitLogEntry() {
    ...
    try {
      ...
      await updateDoc(doc(db, "tasks", selectedDrawerTask.id), {
        logs: updatedLogs,
      });
      ...
      console.error(err);
      toast(
        "Failed to commit log entry: " + (err.message || String(err)),
        "error",
      );
    }
  }
  ...
  async function saveBlueprint() {
    ...
    try {
      await updateDoc(doc(db, "projects", project.id), {
        masterBlueprint: blueprintValue,
      });
  ...
  async function saveLeadInstructions() {
    ...
    try {
      await updateDoc(doc(db, "projects", project.id), {
        leadInstructions: instructionsValue,
      });
  ...
  async function deleteCustomField(id: string) {
    if (!project) return;
    try {
      const updated = (project.customFields || []).filter(
        (f: any) => f.id !== id,
      );
      await updateDoc(doc(db, "projects", project.id), {
        customFields: updated,
  ...
  async function deleteMilestone(id: string) {
    if (!project) return;
    try {
      const updated = (project.milestones || []).filter(
        (m: any) => m.id !== id,
      );
      await updateDoc(doc(db, "projects", project.id), { milestones: updated });
      setProject({ ...project, milestones: updated });
  ...
  if (loading)
    return (
      <div className="p-8 animate-pulse text-sm text-gray-500">
  ...
  const taskAssignees = projectTasks
    .map((t) => t.assignedTo)
    .filter((uid) => uid);
  const allInvolvedUids = Array.from(
    new Set([...projectAssignees, ...taskAssignees]),
  );
  const assignedUsers = users.filter((u) => allInvolvedUids.includes(u.uid));

  const basePaid = project.paid ?? 0;
  const loggedPaid = (project.payments || []).reduce(
    (sum, p) => sum + p.amount,
    0,
  );
  const totalPaid = basePaid + loggedPaid;
    ...
    <div className="p-8">
        ...
        {crmUser?.role === "admin" && (
          <div className="flex flex-wrap items-center gap-4 text-right justify-end">
            <div className="bg-white border border-slate-200/60 shadow-sm rounded-xl px-5 py-4 min-w-[140px]">
              <p className="text-[10px] font-semibold tracking-wider text-slate-500 uppercase mb-1">
                Total Budget
              </p>
      ...
      >
        {[
          ...
          "tasks",
          "files",
          crmUser?.role === "admin" ? "payments" : null,
        ]
          .filter(Boolean)
          .map((tab) => (
            <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {activeTab === "overview" && (
            <div className="space-y-8">
              ...
              <div className="bg-white border border-slate-200/60 rounded-xl p-6 shadow-sm group">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {!editingBlueprint && crmUser?.role === "admin" && (
                    <button
                      onClick={() => {
                        setBlueprintValue(project.masterBlueprint || "");
                ...
                {editingBlueprint ? (
                  ...
                  <p
                    ...
                    style={{ color: "#6b7280" }}
                  >
                    {project.masterBlueprint ||
                      "No master blueprint provided. Click edit to add the overall project architecture."}
                  </p>
                )}
              </div>
              ...
              <div className="bg-white border border-slate-200/60 rounded-xl p-6 shadow-sm group border-l-4 border-l-blue-500">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-bold text-slate-900">
                    Dynamic Lead Instructions
                  </h3>
                  {!editingInstructions && (
                    <button
                      onClick={() => {
                        setInstructionsValue(project.leadInstructions || "");
                  ...
                  >
                    {project.leadInstructions ? (
                      ...
                      <span className="text-slate-400 italic font-medium">
                        No lead instructions currently active for this phase.
                      </span>
                    )}
                  </div>
                )}
                ...
                {editingTechHub ? (
                  <div className="space-y-4">
                      ...
                      <div className="flex flex-wrap gap-2 mt-1">
                          ...
                          >
                            ...
                            <button
                              ...
                              onClick={() => {
                                setTechHubForm({
                                  ...techHubForm,
                                  techStack: techHubForm.techStack.filter(
                                    (t) => t !== tech,
                                  ),
                                });
                              }}
                        ...
                        {techHubForm.techStack.length === 0 && (
                          <span className="text-xs text-slate-400 italic py-1">
                            No custom technologies defined
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          ...
                          <input
                            className="form-input"
                            value={techHubForm.productionUrl}
                            onChange={(e) =>
                              setTechHubForm({
                                ...techHubForm,
                    ...
                    <div className="flex gap-2 pt-2">
                      <button
                        onClick={saveTechnicalHub}
                        className="btn-primary py-1.5 px-4 text-xs"
                      >
                        Save Provisioning
                      </button>
                      <button
                        onClick={() => setEditingTechHub(false)}
                        className="px-4 py-1.5 rounded-lg text-xs font-bold text-slate-500 hover:bg-slate-100 transition-colors"
                      >
                        Cancel
                      </button>
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {[
                        {
                          label: "Figma Canvas URL",
                          val: (project as any).figmaUrl,
                        },
                        {
                          label: "Repository Endpoint",
                          val: (project as any).repoUrl,
                        },
                        {
                          label: "Staging Environment",
                          val: (project as any).stagingUrl,
                        },
                        {
                          label: "Production Endpoint",
                          val: (project as any).productionUrl,
                        },
                      ].map((urlObj, index) => (
                        <div key={index} className="space-y-1.5">
                          ...
                          <div
                            className={`text-xs border border-slate-200/60 p-3 rounded-lg flex items-center justify-between transition-all shadow-sm ${urlObj.val ? "bg-white" : "bg-slate-50/80"}`}
                          >
                            <span
                              className={`truncate mr-2 ${urlObj.val ? "text-slate-700 font-mono font-medium tracking-tight" : "text-slate-400 font-medium italic"}`}
                            >
                    ...
                    <div className="flex items-start justify-between pt-4 mt-2 flex-wrap gap-6 border-t border-slate-100 dark:border-slate-800">
                      <div>
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1.5 pl-1">
                          Primary Focus Scope
                        </span>
                        {editingCoreFocus ? (
                          <input
                      ...
                      <div className="text-right">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1.5 pr-1">
                          Tech Stack Matrix
                        </span>
                        <div className="flex flex-wrap justify-end gap-1.5">
                          {((project as any).techStack || []).length === 0 ? (
                            ...
                            ((project as any).techStack || []).map(
                              (t: string) => (
                                <span
                                  key={t}
                                  className="text-xs font-bold bg-slate-100 dark:bg-[#0D1B3E]/40 border border-slate-200 dark:border-[#0D1B3E]/60 text-[#0D1B3E] dark:text-slate-300 px-2.5 py-1 rounded-md shadow-sm"
                                >
                                  {t}
                                </span>
                              ),
              ...
              {(() => {
                ...
                const progressMean =
                  totalTasksCount > 0
                    ? Math.round(
                        projectTasks.reduce(
                          (sum, t) => sum + getTaskProgressVal(t.status),
                          0,
                        ) / totalTasksCount,
                      )
                    : 0;
                  ...
                  <div className="bg-white border border-slate-200/60 rounded-xl p-6 shadow-sm">
                    ...
                    <div className="relative flex items-center justify-between mt-10 mb-16 px-4 md:px-8">
                      ...
                      <div
                        ...
                        style={{
                          width: `calc(${Math.max(0, Math.min(100, progressMean))}% - 4rem)`,
                        }}
                      />
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="absolute top-10 left-1/2 -translate-x-1/2 text-center w-32 flex flex-col items-center justify-start pt-1">
                              <p
                                className={`text-[10px] font-semibold tracking-wide leading-tight ${isActive ? "text-slate-800" : isCompleted ? "text-slate-600" : "text-slate-400"}`}
                              >
                                {phase.label}
                              </p>
                    ...
                    <div className="mt-8 text-center text-xs font-semibold text-slate-500 border-t pt-4 border-slate-50">
                      Overall Progress Mean:{" "}
                      <span className="text-blue-600 font-bold text-sm ml-1">
                        {progressMean}%
                      </span>{" "}
                      based on {totalTasksCount} tasks.
                    </div>
                  </div>
                );
              })()}
                ...
                {projectTasks.length === 0 ? (
                  <div className="text-center py-12 bg-white border border-slate-200/60 rounded-xl shadow-sm">
                    <p className="text-sm font-medium text-slate-500">
                      No tasks delegated to this workspace yet.
                    </p>
                  </div>
                ) : (
                  projectTasks.map((task) => {
                    const pct = Math.round(
                      (getMilestoneIndex(task.status) / 4) * 100,
                    );

                    // Priority configurations
                    let pBg = "bg-slate-100 text-slate-600 border-slate-200";
                    if (task.priority === "high")
                      pBg = "bg-rose-50 text-rose-700 border-rose-200/60";
                    else if (task.priority === "medium")
                      pBg = "bg-amber-50 text-amber-700 border-amber-200/60";
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center space-x-4 min-w-[240px]">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1.5">
                              <span
                                className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full border ${pBg}`}
                              >
                        ...
                        <div
                          className="flex-1 max-w-xl w-full px-4 py-2.5 bg-slate-50 rounded-lg border border-slate-200/50"
                          ...
                          {(crmUser?.role === "admin" ||
                            ...
                            >
                              ...
                              <select
                                ...
                                onChange={(e) =>
                                  reallocateAsset(task, e.target.value)
                                }
                ...
                <div className="flex flex-col md:flex-row gap-4 items-center">
                  ...
                  <div className="flex items-center gap-4 w-full md:w-auto">
                    ...
                    <button
                      onClick={addAsset}
                      disabled={
                        addingFile || !assetName.trim() || !assetUrl.trim()
                      }
                ...
                <div className="space-y-3">
                  {(() => {
                    ...
                    return filtered.map((file: any, index: number) => {
                      ...
                      const isGithub =
                        file.url.includes("github.com") ||
                        file.url.includes("gitlab.com") ||
                        file.url.includes("bitbucket");
                      ...
                      const cat =
                        file.category?.toUpperCase() || "DOCUMENTATION";
                      let badgeClasses =
                      ...
                      return (
                        <div
                          key={index}
                          className="flex items-center justify-between p-5 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 shadow-sm hover:shadow-md transition-all group"
                        >
                          <div className="flex items-center gap-4 truncate">
                            ...
                            <div className="truncate">
                              ...
                              <p className="text-[11px] font-mono text-slate-400 dark:text-slate-500 truncate max-w-xs md:max-w-md lg:max-w-lg mb-1">
                                {file.url}
                              </p>
                              <p className="text-[10px] font-medium text-slate-400">
                                Added by {file.addedBy} &bull;{" "}
                                {new Date(file.at).toLocaleDateString("en-GB")}
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center gap-4 flex-shrink-0 ml-4">
                            <span
                              className={`hidden md:inline-block px-2.5 py-1 rounded-md text-[9px] font-black tracking-widest border ${badgeClasses}`}
                            >
                              {cat}
                            </span>
                            <a
              ...
              <div className="crm-card">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      Track and manage financial transactions for this project.
                    </p>
                  </div>
                  <div className="relative group">
                    ...
                    <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                      ...
                      >
                        <span className="text-sm">🔔</span> Send Payment
                        Reminder
                      </button>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                  <div className="p-4 rounded-xl border border-slate-100 bg-slate-50">
                    <p className="text-[10px] font-bold text-slate-500 mb-1 uppercase tracking-wider">
                      Total Budget
                    </p>
                    <p className="text-lg font-black text-[#0D1B3E]">
                      {project.currency || "AED"}{" "}
                      {project.budget?.toLocaleString() || "0"}
                    </p>
                  </div>
                  <div className="p-4 rounded-xl border border-orange-100 bg-orange-50">
                    <p className="text-[10px] font-bold text-orange-600 mb-1 uppercase tracking-wider">
                      Current Due
                    </p>
                  ...
                  <div className="p-4 rounded-xl border border-green-100 bg-green-50">
                    <p className="text-[10px] font-bold text-green-600 mb-1 uppercase tracking-wider">
                      Total Paid
                    </p>
                  ...
                  <div className="p-4 rounded-xl border border-red-100 bg-red-50">
                    <p className="text-[10px] font-bold text-red-600 mb-1 uppercase tracking-wider">
                      Remaining
                    </p>
                ...
                <div className="mb-8">
                  ...
                  <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, (totalPaid / (project.budget || 1)) * 100)}%`,
                      }}
                    ></div>
                  </div>
                </div>
                      ...
                      <table className="w-full text-left border-collapse text-sm">
                        <thead>
                          <tr className="bg-slate-50 border-b border-slate-200">
                            <th className="p-4 font-bold text-slate-500 text-xs">
                              Date
                            </th>
                            <th className="p-4 font-bold text-slate-500 text-xs">
                              Method
                            </th>
                            <th className="p-4 font-bold text-slate-500 text-xs text-right">
                              Amount
                            </th>
                          </tr>
                        </thead>
                              ...
                              >
                                <td className="p-4 text-slate-900 font-medium">
                                  ...
                                  <p className="text-[9px] text-slate-300 mt-1">
                                    Logged by {payment.loggedBy}
                                  </p>
                                </td>
                                <td className="p-4 text-slate-600">
                                  <span className="bg-slate-100 px-2 py-1 rounded text-xs font-bold uppercase tracking-wider">
                                    {payment.method}
                                  </span>
                                </td>
        ...
        <div className="space-y-6">
          <div className="crm-card">
            <h3 className="text-sm font-bold mb-4" style={{ color: "#0D1B3E" }}>
              Team Members
            </h3>
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={finForm.due}
                    onChange={(e) =>
                      setFinForm({ ...finForm, due: e.target.value })
                    }
                  />
                </div>
                <div>
            ...
            <div className="flex gap-4 mt-8">
              ...
              >
                {sendingReminder ? (
                  ...
                  <>
                    <span>✉️</span> Send Reminder Email
                  </>
                )}
              </button>
            </div>
          ...
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="flex gap-3 mt-8">
              ...
              <button
                ...
                disabled={
                  delegating || !delegateForm.employeeId || !delegateForm.title
                }
                className="flex-1 py-3 text-sm font-bold text-white bg-[#0D1B3E] rounded-xl hover:opacity-90 disabled:opacity-50"
              >
          ...
          <div className="bg-white rounded-3xl p-8 max-w-lg w-full shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  This employee is already assigned to an active task on this
                  project:{" "}
                  <strong className="text-slate-800">
                    {duplicateConflictTask.title}
                  </strong>
                </p>
              </div>
              <button
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1 group-hover:text-[#C9A84C]">
                    Option B
                  </h4>
                  ...
                  <p className="text-[9px] text-slate-400 mt-1">
                    Open detail drawer to adjust deadlines or parameters.
                  </p>
                </button>
                ...
                >
                  ...
                  <p className="text-[9px] text-slate-400 mt-1">
                    Return to dropdown to pick another team asset.
                  </p>
                </button>
              </div>
            </div>
        ...
        {selectedDrawerTask &&
          (() => {
            ...
            const summary =
              ...
              "No master brief provided by Admin.";
            const instructions =
              selectedDrawerTask.taskInstructions ||
                          ...
                          <div className="flex-1 overflow-y-auto space-y-3 pr-1 max-h-[300px]">
                            {logs.length === 0 ? (
                              <p className="text-xs text-slate-400 italic text-center py-8">
                                No committed log entries found.
                              </p>
                            ) : (
                              logs
                                .slice()
