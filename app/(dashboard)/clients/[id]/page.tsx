...
export default function ClientProfilePage({
  ...
  useEffect(() => {
    ...
    const fetchRelatedData = async () => {
      try {
        const [propSnap, projSnap, taskSnap, invSnap, membersSnap] =
          await Promise.all([
            getDocs(
              query(
        ...
        setProposals(
          propSnap.docs.map((d) => ({ id: d.id, ...d.data() }) as Proposal),
        );
  ...
  async function submitQuickTask() {
    ...
    try {
      ...
      const taskSnap = await getDocs(
        query(collection(db, "tasks"), where("clientId", "==", params.id)),
      );
      setTasks(taskSnap.docs.map((d) => ({ id: d.id, ...d.data() }) as Task));
    } catch (e: any) {
