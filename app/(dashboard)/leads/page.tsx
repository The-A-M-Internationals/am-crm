...
export default function LeadsPage() {
  ...
  function openEdit(lead: Lead) {
    ...
    setForm({
      ...
      stage: lead.stage,
      lifecycleStatus: (lead as any).lifecycleStatus || "Not Contacted",
      followUpDate: lead.followUpDate ?? "",
      notes: lead.notes ?? "",
      source: lead.source ?? "",
      nextAction: (lead as any).nextAction ?? "",
    });
    setIsCustomAction(false);
    setShowModal(true);
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      const leadData = {
        ...form,
        id: leadId,
        assignedTo: editing?.assignedTo ?? crmUser?.uid ?? "",
      ...
      if (form.stage === "won") await PipelineService.markAsWon(leadData);
        ...
        await PipelineService.markAsLost(leadId, form.email, "lead");
      else if (form.stage === "proposal") {
        await PipelineService.transitionToProposal(
          leadData,
          crmUser?.uid ?? "",
        );
  ...
  async function moveStage(lead: Lead, stage: LeadStage) {
    try {
      if (stage === "won") await PipelineService.markAsWon(lead);
        ...
        await PipelineService.markAsLost(lead.id, lead.email, "lead");
  ...
  const svcInfo = (key: string) =>
    SERVICES.find((s) => s.key === key) ?? {
      key: "other",
      label: key?.toUpperCase() || "OTHER",
      bg: "#f3f4f6",
      text: "#374151",
    };
  const stgInfo = (key: string) =>
    STAGES.find((s) => s.key === key) ?? {
      key: "lead",
      label: key?.toUpperCase() || "LEAD",
      color: "#7e22ce",
      bg: "#faf5ff",
      border: "#e9d5ff",
    };
  ...
  const inProgress = leads.filter(
    (l) => l.active !== false && l.stage !== "won" && l.stage !== "lost",
  ).length;
        ...
        {loading ? (
          <div className="flex gap-4 h-full">
            {[1, 2, 3, 4, 5].map((i) => (
              <div
                key={i}
                className="w-[320px] flex-shrink-0 bg-slate-100 rounded-2xl animate-pulse h-full"
              />
            ))}
          </div>
        ) : (
            ...
            {STAGES.map((stage) => {
              ...
              return (
                ...
                >
                  {/* Column Header */}
                  <div
                    className="px-4 py-3 border-b flex items-center justify-between bg-white rounded-t-2xl"
                    ...
                    {stageLeads.map((lead) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Contact Info */}
                          <div className="mb-3">
                            <h4 className="font-bold text-[15px] text-slate-900 leading-tight mb-0.5 group-hover:text-blue-600 transition-colors">
                              {lead.company}
                            </h4>
                            <p className="text-xs font-medium text-slate-500 flex items-center gap-1.5">
                              <span className="w-4 h-4 rounded-full bg-slate-100 flex items-center justify-center text-[8px]">
                                👤
                              </span>
                              {lead.name}
                            </p>
                          ...
                          <div className="flex gap-1.5 pt-2 border-t border-slate-100 overflow-x-auto pb-1 no-scrollbar opacity-0 group-hover:opacity-100 transition-opacity">
                            {STAGES.filter((s) => s.key !== lead.stage).map(
                              (s) => (
                                <button
                                  key={s.key}
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    moveStage(lead, s.key);
                                  }}
          ...
          <div className="modal-box shadow-2xl" style={{ maxWidth: 600 }}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="form-label text-xs font-bold uppercase tracking-wide text-slate-500">
                    Full Name *
                  </label>
                  <input
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        service: e.target.value as ServiceTag,
                      })
                    }
                  >
                    {SERVICES.map((s) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    className="form-input"
                    value={(form as any).lifecycleStatus || "Not Contacted"}
                    onChange={(e) =>
            ...
            {form.stage === "won" && (
              <div className="mt-4 px-4 py-3 rounded-xl text-xs font-medium bg-blue-50 text-blue-800 border border-blue-200">
                ℹ️ Marking as <strong>Won</strong> tags this as a successful
                lead. To convert them to an active <strong>Client</strong>, you
                must generate and accept a Proposal.
              </div>
            )}
            ...
            <div className="flex items-center justify-between mt-8 pt-4 border-t border-slate-100">
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={handleSave}
                  disabled={
                    saving || !form.name || !form.company || !form.email
                  }
                  className="btn-primary px-8 disabled:opacity-50 shadow-md hover:shadow-lg transition-all"
                >
                  {saving
                    ? "Saving..."
                    : editing
