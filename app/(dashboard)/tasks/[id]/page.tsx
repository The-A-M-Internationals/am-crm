...
export default function TaskOperationalSheet({
  params,
}: {
  params: { id: string };
}) {
  ...
  const [activeTab, setActiveTab] = useState<"blueprints" | "chat">(
    "blueprints",
  );
    ...
    return (
      <div className="p-10 text-center flex flex-col items-center">
        ...
        <p className="text-slate-500 mb-6">
          This task belongs to another team member.
        </p>
        <button
  ...
  const summary =
    task.masterBlueprint ||
    task.projectSummary ||
    "No master brief provided by Admin.";
  const instructions =
    task.leadInstructions ||
              ...
              <div className="bg-slate-50 border-b border-slate-200 p-5 shrink-0 z-10 shadow-sm relative">
                ...
                <div className="relative h-14 w-full bg-white p-3 rounded-2xl border border-slate-100 shadow-inner flex items-center justify-between px-6">
                  ...
                  <div
                    ...
                    style={{
                      width: `calc(${localProgress}% - ${localProgress === 100 ? 4 : localProgress === 0 ? 0 : 2}rem)`,
                    }}
                  />
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-100/50">
                {!task.activityLogs || task.activityLogs.length === 0 ? (
                  <div className="h-full flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="text-slate-400 text-sm font-medium">
                        The communication hub is silent.
                      </p>
                      <p className="text-slate-400 text-xs mt-1">
                        Send the first update to alert the team.
                      </p>
                    </div>
                  </div>
                ) : (
                  task.activityLogs.map((log: any) => {
                    ...
                    const isManager =
                      log.authorRole === "admin" || log.authorRole === "lead";
                      ...
                      >
                        ...
                        >
                          <div className="flex justify-between items-start gap-4 mb-2">
                            <span
                              className={`text-[9px] font-black px-2 py-0.5 rounded shadow-sm flex items-center gap-1 ${
                                isMine
                            ...
                            <div className="mt-2 flex flex-col gap-2 min-w-[250px]">
                              ...
                              <div className="flex gap-2 justify-end">
                                <button
                                  ...
                                  className="px-2 py-1 rounded bg-blue-800 text-[10px] font-bold hover:bg-blue-900 transition-colors"
                                >
                                  Cancel
                                </button>
                                <button
                                  onClick={() => handleSaveLogEdit(log.id)}
                                  disabled={!editingLogText.trim()}
                                  className="px-2 py-1 rounded bg-white text-blue-600 text-[10px] font-bold hover:bg-blue-50 transition-colors"
                                >
                                  Save
                                </button>
                              </div>
                          ...
                          {log.isEdited && (
                            <div
                              className={`text-[8px] font-bold italic mt-2 text-right ${isMine ? "text-blue-300" : "text-slate-400"}`}
                            >
                              (edited)
                            </div>
                          )}
